...
description: "Manages dependency updates for the project. Before touching any dependency, checks that unit test coverage is at least 80% — if not, delegates to orangit_tester to bring coverage up first. Then iteratively scans (Syft/Grype), applies patch and minor updates, rebuilds, and rescans until all fixable High and Critical vulnerabilities are resolved. Never performs major version upgrades — hand those to orangit_upgrader."
model: sonnet
memory: project
---
